...
static struct list sleep_list; // for blocked sleeping threads
  ...
  if (!list_empty (&t->locks_held))
    {
      ...
      struct lock *l = list_entry (list_front (&t->locks_held), struct lock, elem);
  ...
  for (int depth = 0; depth < 8; depth++)
    {
      ...
      /* Update lock's max_priority to ensure persistence of donation. */
      if (cur->priority > lock->max_priority)
        lock->max_priority = cur->priority;

      /* Only update holder's priority if we are boosting it. 
         This handles multiple donations (respecting the max). */
      if (holder->priority < cur->priority)
        {
          holder->priority = cur->priority;
          
          /* Re-sort the list the holder is in, if any. */
...
thread_init (void) 
{
  ...
  threadminTick = INT64_MAX; // Set to the max possible value initially
...
// Comparator function to insert based on wake-up time
bool wake_up_time_less(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED) {
    struct thread *ta = list_entry(a, struct thread, elem);
    struct thread *tb = list_entry(b, struct thread, elem);

    return ta->waketick < tb->waketick;  // Insert in ascending order of wake-up time
}
/*
...
void thread_sleep(int64_t waketick){
  ...
  if(cur != idle_thread){
    ...
    //update global tick if this new wait time is lower
    if(waketick < threadminTick){
      threadminTick = waketick;
    }
    //put current thread into the blocked state
    cur->status = THREAD_BLOCKED;
    //put current thread onto the sleep list NEEDS INSERT ORDERED
    list_insert_ordered(&sleep_list, &cur->elem, wake_up_time_less , NULL);
    //schedule the next thread while we wait
    schedule ();
...
void thread_wake() {
    ...
    while (e != list_end(&sleep_list)) {
        ...
        // If the thread's wake time has arrived or passed
